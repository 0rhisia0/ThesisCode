...
"""Skew normal function for fitting and evaluation putposes """
...
def fitToS1S2(data):
    a = np.arange(1.5, 98.5, 1)
    b = np.arange(2.5, 99.5, 1)
    S1 = data[0, :]
    S2 = data[1, :]
    xi = np.zeros(len(a))
    ome = np.zeros(len(a))
    ...
    for i in tqdm(range(len(b))):
        ...
        popt, pcov = curve_fit(skewNormNew, bin_centers, y, p0=[4, 0.1, 0, norm_fact], bounds=([logS2Min, 0., -3., 0.], [logS2Max, 1., 3., 1e7]))
...
def main():
    # data = getData()
    # print(data.shape)
    data = np.load('Data/NR_Fit/GregNR.npy')
    fitToS1S2(data)

    # compare(datFfa1, data2)
    # Parameter fits
    a = np.load('Data/NR_Fit/fit_data.npy')
    print(a[0])
    input("hello")
    # fit epsilon
    ...
    print(opts)
    np.save("Data/NR_Fit/NR_popts", opts)
